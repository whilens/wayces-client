...
type ProductCardItem = { id: number; name: string };
type FiltersState = {
...
const Products = () => {
  ...
  const items = useAppSelector(selectProducts) as ProductCardItem[];
  const isLoading = useAppSelector(selectProductsIsLoading);
  const error = useAppSelector(selectProductsError);
  const pagination = useAppSelector(selectProductsPagination);
  const filters = useAppSelector((state) => state.products.filters as FiltersState);
  ...
  const [productsForFilters, setProductsForFilters] = useState<ProductCardItem[]>([]);
  ...
  useEffect(() => {
    ...
    const loadProducts = async () => {
      ...
      try {
        const fetchProductsThunk = fetchProducts as unknown as (
          payload: Record<string, string | number | boolean>
        ) => unknown;
        const dispatched = dispatch(fetchProductsThunk(params) as never) as unknown as {
          unwrap: () => Promise<{ hasMore?: boolean; products?: ProductCardItem[] }>;
        };
        const result = await dispatched.unwrap();
  ...
  const productsList = useMemo(
    () =>
      items.map((product: ProductCardItem) => (
