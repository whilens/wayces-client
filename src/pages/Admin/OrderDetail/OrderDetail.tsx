...
const AdminOrderDetail = () => {
  ...
  const fetchOrder = async () => {
    try {
      ...
      setError(getErrMsg(err, 'Ошибка загрузки заказа'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStatusChange = async (newStatus: OrderStatus) => {
    try {
      ...
      notification.error({
        message: 'Ошибка',
        description: getErrMsg(err, 'Не удалось обновить статус'),
        placement: 'topRight',
      });
    } finally {
      setIsUpdating(false);
            ...
            <div className="admin-order-detail__status-buttons">
              {Object.entries(ORDER_STATUS).map(([key, value]) => (
                <button
                  ...
                  disabled={isUpdating || order.status === (value as OrderStatus)}
                >
