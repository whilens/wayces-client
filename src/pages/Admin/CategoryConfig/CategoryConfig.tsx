...
type VariantOptionValue = { value: string; colorCode?: string | null };
...
type SpecModalForm = { key: string; label: string; type: string; options: string; displayOrder: number; unit: string };
type VariantModalForm = {
  key: string;
  name: string;
  type: string;
  isRequired: boolean;
...
const toErrMsg = (error: unknown, fallback: string): string => {
...
const CategoryConfig = () => {
  ...
  const [specifications, setSpecifications] = useState<CategorySpec[]>([]);
  ...
  const loadConfig = async () => {
    try {
      ...
      notification.error({
        message: 'Ошибка',
        description: toErrMsg(error, 'Не удалось загрузить конфигурацию'),
        placement: 'topRight',
      });
    } finally {
      setIsLoading(false);
  ...
  const handleEditSpecification = (spec: CategorySpec) => {
    setEditingSpec(spec);
    setIsSpecModalOpen(true);
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          notification.error({
            message: 'Ошибка',
            description: toErrMsg(error, 'Не удалось удалить характеристику'),
            placement: 'topRight',
          });
        }
      },
  ...
  const handleSaveSpecification = async (values: {
    ...
    try {
      ...
      notification.error({
        message: 'Ошибка',
        description: toErrMsg(error, 'Не удалось сохранить характеристику'),
        placement: 'topRight',
      });
    }
  };
  ...
  const handleEditVariant = (variant: CategoryVariant) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          notification.error({
            message: 'Ошибка',
            description: toErrMsg(error, 'Не удалось удалить вариант'),
            placement: 'topRight',
          });
        }
      },
  ...
  const handleSaveVariant = async (values: {
    ...
    displayOrder: number;
    unit: string | null;
    optionValues: Array<{ value: string; colorCode: string | null }>;
  }) => {
    try {
      if (editingVariant) {
      ...
      notification.error({
        message: 'Ошибка',
        description: toErrMsg(error, 'Не удалось сохранить вариант'),
        placement: 'topRight',
      });
    }
  };
...
const SpecificationModal = ({
  ...
  const [formData, setFormData] = useState<SpecModalForm>({
    key: '',
    label: '',
    type: 'text',
    options: '',
...
const VariantModal = ({
  ...
  const [formData, setFormData] = useState<VariantModalForm>({
    key: '',
    name: '',
    type: 'button',
    isRequired: true,
  ...
  const isColorVariant = (key: string, type: string) => key === 'color' || type === 'color';
  ...
  useEffect(() => {
    ...
    if (variant) {
      ...
      setFormData({
        ...
        optionValues:
          raw.length
            ? raw.map((o: VariantOptionValue, i: number) => {
  ...
  const updateOptionValue = (index: number, field: keyof VariantOptionValue, value: string) => {
  ...
  const pendingColorsRef = useRef<Record<number, string>>({});
