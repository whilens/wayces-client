...
const Checkout = () => {
  ...
  const { isAuthenticated, user, authModal: globalAuthModal } = useAppSelector((state) => state.user);
    ...
    if (isAuthenticated && isSubmitting && wasAuthModalOpen && !globalAuthModal.isOpen) {
      ...
      const submitOrder = async () => {
        ...
        try {
          ...
          notification.error({
            message: 'Ошибка',
            description: error.response?.data?.message || 'Произошла ошибка при оформлении заказа. Попробуйте позже.',
            placement: 'topRight',
            duration: 5,
          });
        } finally {
