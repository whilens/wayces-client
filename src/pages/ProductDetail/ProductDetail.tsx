...
type VariantGroup = { name: string; type: string; options: VariantOption[]; default?: string };
type ProductVariantMap = Record<string, VariantGroup>;
...
type ProductData = {
  ...
  sku?: string;
  categoryId?: number;
  categoryConfig?: { specLabelsMap?: Record<string, string>; specifications?: Array<{ key: string; label?: string; unit?: string }> };
...
type DiscountPriceInfo = { hasDiscount: boolean; originalPrice: number; discountedPrice: number };
type SimilarProduct = {
  id: number;
  name?: string;
  basePrice?: number;
...
type ProductsState = { currentProduct: ProductData | null; isLoading: boolean; error: unknown };

const ProductDetail = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useAppDispatch();
  const { currentProduct, isLoading, error } = useAppSelector((state) => state.products as ProductsState);
  ...
  const [categoryConfig, setCategoryConfig] = useState<ProductData['categoryConfig'] | null>(null);

  useEffect(() => {
    if (id) {
      // Очищаем предыдущий товар перед загрузкой нового
      dispatch(clearCurrentProduct());
      const fetchProductThunk = fetchProductById as unknown as (payload: string) => unknown;
      dispatch(fetchProductThunk(id) as never);
  ...
  useEffect(() => {
    ...
    } else if (currentProduct?.categoryId != null) {
  ...
  const specLabelsMap = useMemo(() => {
    ...
    categoryConfig.specifications.forEach((spec: { key: string; label?: string }) => {
  ...
  const specUnitsMap = useMemo(() => {
    ...
    const map: Record<string, string> = {};
    categoryConfig.specifications.forEach((spec: { key: string; unit?: string }) => {
  ...
  const prepareCartItem = useCallback(() => {
    ...
    const priceToUse =
      currentCombination?.price != null ? parseFloat(String(currentCombination.price)) : finalPrice;
    const priceInfoCart = calcDiscountedPrice(
      priceToUse,
        ...
        <div className="product-detail__content">
          ...
          <div className="product-detail__info">
            ...
            <div className="product-detail__actions">
              {(currentProduct.combinations?.length ?? 0) > 0 && !canAddToCart && (
