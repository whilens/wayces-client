 ...
 */
export const getMockProducts = async (params: MockQuery = {}) => {
  ...
  const allProducts = mockProductsWithVariants.length > 0
    ? [
        ...(mockProductsWithVariants as MockProduct[]),
        ...(mockProducts as MockProduct[]).filter(
          (p) => !(mockProductsWithVariants as MockProduct[]).find((v) => v.id === p.id)
        ),
      ]
    : [...mockProducts];

  // Генерируем все комбинации вариантов
  let products = generateAllProductCombinations(allProducts as unknown as Parameters<typeof generateAllProductCombinations>[0]);
 ...
 */
export const getMockProductById = async (id: number | string) => {
  await delay(200);
  ...
  let product = (mockProductsWithVariants as Array<Record<string, unknown>>).find(
    (p) => Number(p.id) === productId
  );
  
  // Если не нашли, ищем в старых данных
  if (!product) {
    product = (mockProducts as Array<Record<string, unknown>>).find((p) => Number(p.id) === productId);
  }
